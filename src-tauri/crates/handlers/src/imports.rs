// Transport-agnostic import handlers shared by the Tauri shims and the Axum
// server. Pre-lift these lived in `src-tauri/src/commands/storage/imports.rs`
// and took the Tauri `AppState`. The lifted versions take an explicit
// (`storage: &FileStorage`, `data_dir: &Path`) pair so the same code runs from
// either binary.
...
use marinara_core::{ensure_object, new_id, now_millis, AppError, AppResult};
...
pub fn resolve_import_folder(body: &Value) -> AppResult<PathBuf> {
    let raw_path = body
        .get("folderPath")
...
pub fn import_payload(body: Value) -> AppResult<Value> {
...
pub(super) fn create_lorebook_from_payload(
    storage: &FileStorage,
    _data_dir: &Path,
    payload: &Value,
    fallback_name: &str,
    ...
    let record = storage.create("lorebooks", lorebook)?;
...
fn patch_imported_character_lorebook_pointer(
    ...
    let character = get_required(storage, "characters", character_id)?;
...
pub(super) fn import_st_character_payload(
    storage: &FileStorage,
    data_dir: &Path,
    payload: Value,
    ...
    let character = storage.create("characters", record)?;
...
pub fn import_st_character(
    ...
    import_st_character_payload(storage, data_dir, payload, None, &body)
}

fn import_st_character_batch(
    storage: &FileStorage,
    data_dir: &Path,
    body: Value,
) -> AppResult<Value> {
...
fn import_marinara_file(storage: &FileStorage, data_dir: &Path, body: Value) -> AppResult<Value> {
    let uploaded = decode_uploaded_file_value(
        body.get("file")
            .ok_or_else(|| AppError::invalid_input("file is required"))?,
    )?;
    ...
    import_marinara_envelope(storage, data_dir, envelope)
}
...
fn restore_sprites(
    ...
    let dir = data_dir.join("sprites").join(target_id);
...
fn import_marinara_character(
    ...
    if data.get("spec").is_some() && data.get("data").is_some_and(Value::is_object) {
        ...
        let character_id = character
            ...
            .to_string();
        let sprites_imported =
            restore_sprites(storage, data_dir, &character_id, data.get("sprites"))?;
        let gallery_imported =
            restore_character_gallery(storage, data_dir, &character_id, data.get("gallery"))?;
    ...
    if !looks_like_storage_record {
        return import_st_character_payload(storage, data_dir, data, None, &Value::Null);
    }
    ...
    let record = storage.create("characters", record_value)?;
    let character_id = record
        .get("id")
        .and_then(Value::as_str)
        .ok_or_else(|| AppError::new("storage_error", "Created character is missing an id"))?
        .to_string();
    let sprites_imported = restore_sprites(storage, data_dir, &character_id, data.get("sprites"))?;
    let gallery_imported =
        restore_character_gallery(storage, data_dir, &character_id, data.get("gallery"))?;
...
fn import_marinara_persona(
    ...
    hydrate_metadata_timestamps(&mut timestamp_payload);
    apply_timestamp_overrides(&mut record_value, &Value::Null, &timestamp_payload);
    let record = storage.create("personas", record_value)?;
    let persona_id = record
        .get("id")
        .and_then(Value::as_str)
        .ok_or_else(|| AppError::new("storage_error", "Created persona is missing an id"))?
        .to_string();
    let sprites_imported = restore_sprites(storage, data_dir, &persona_id, data.get("sprites"))?;
    Ok(json!({
        "success": true,
...
fn import_marinara_lorebook(
    ...
    for folder in array_from_envelope(&data, envelope, "folders") {
        ...
        let created = storage.create("lorebook-folders", Value::Object(folder_record))?;
    ...
    for (index, entry) in exported_entries.iter().enumerate() {
        ...
        storage.create("lorebook-entries", normalized)?;
    }
...
fn import_marinara_preset(
    ...
    hydrate_metadata_timestamps(&mut timestamp_payload);
    apply_timestamp_overrides(&mut record_value, &Value::Null, &timestamp_payload);
    let record = storage.create("prompts", record_value)?;
    ...
    for group in array_from_envelope(&data, envelope, "groups") {
        ...
        let created = storage.create("prompt-groups", Value::Object(group_record))?;
        if let (Some(old_id), Some(new_id)) = (
            old_id,
            created
                .get("id")
    ...
    for section in array_from_envelope(&data, envelope, "sections") {
        ...
        storage.create("prompt-sections", Value::Object(section_record))?;
        sections_imported += 1;
    }
    ...
    for variable in variables {
        ...
        storage.create("prompt-variables", Value::Object(variable_record))?;
...
pub fn import_st_preset_payload(
    storage: &FileStorage,
    data_dir: &Path,
    raw: Value,
    ...
    let (group_id_map, group_ids) =
        create_st_prompt_groups(storage, data_dir, &preset_id, &sorted_prompts)?;
...
fn import_marinara_envelope(
    ...
    match import_type {
        "marinara_character" => import_marinara_character(storage, data_dir, data),
        "marinara_persona" => import_marinara_persona(storage, data_dir, data),
        "marinara_lorebook" => import_marinara_lorebook(storage, data_dir, object, data),
        "marinara_preset" => import_marinara_preset(storage, data_dir, object, data),
...
pub fn import_call(
    ...
    match rest {
        ["marinara"] => {
            ...
            import_marinara_envelope(storage, data_dir, payload)
        }
        ["marinara-file"] => import_marinara_file(storage, data_dir, body),
        ["st-character"] => import_st_character(storage, data_dir, body),
        ["st-character", "batch"] => import_st_character_batch(storage, data_dir, body),
        ["st-character", "inspect"] => inspect_st_character_batch(body),
        ["st-chat"] => bulk_imports::import_st_chat(storage, data_dir, body),
        ["st-chat-into-group"] => bulk_imports::import_st_chat_into_group(storage, data_dir, body),
        ["st-preset"] => {
            let payload = import_payload(body)?;
            let filename = payload
                ...
                .and_then(Value::as_str)
                .map(ToOwned::to_owned);
            import_st_preset_payload(storage, data_dir, payload, filename.as_deref())
        }
        ["st-lorebook"] => {
            let payload = import_payload(body)?;
        ...
        ["st-bulk", "run"] => bulk_imports::run_st_bulk_import(storage, data_dir, body),
        _ => Err(AppError::new(
            "route_not_found",
...
mod tests {
    ...
    #[test]
    fn import_st_character_creates_character_record() {
        let (dir, storage) = setup();
        // ST v2 card body (no embedded file), the import is meant to accept a
        // raw character object when no `file` is included.
        let body = json!({
            "spec": "chara_card_v2",
            "spec_version": "2.0",
            "data": {
                "name": "Ada",
                "description": "An AI navigator",
                "personality": "Curious",
        ...
        let result = import_st_character(&storage, dir.path(), body).expect("import");
        assert_eq!(result["success"], true);
        let character_id = result["characterId"].as_str().expect("characterId");
        let stored = storage
            .get("characters", character_id)
            .expect("storage get")
            .expect("character exists");
        assert_eq!(stored["format"], "chara_card_v2");
        let raw_data = stored["data"].as_str().expect("data is serialized string");
        let parsed: Value = serde_json::from_str(raw_data).expect("data is valid JSON");
        assert_eq!(parsed["name"], "Ada");
        assert_eq!(parsed["tags"], json!(["sci-fi"]));
    }

    #[test]
    fn import_marinara_envelope_for_lorebook_creates_entries() {
        let (dir, storage) = setup();
        let envelope = json!({
            "version": 1,
            "type": "marinara_lorebook",
            "data": {
                ...
                "entries": [
                    { "key": ["dragon"], "content": "Dragons exist." },
                    { "key": ["tower"], "content": "There is a tower." }
                ]
            }
        });
        let result = import_call(&storage, dir.path(), &["marinara"], envelope).expect("import");
        assert_eq!(result["type"], "marinara_lorebook");
        assert_eq!(result["entriesImported"], 2);
        let lorebook_id = result["lorebookId"].as_str().expect("lorebookId");
        let entries = storage.list("lorebook-entries").expect("entries");
        assert_eq!(entries.len(), 2);
        for entry in &entries {
            assert_eq!(entry["lorebookId"], json!(lorebook_id));
        }
    }

    #[test]
    fn import_call_rejects_unsupported_envelope_version() {
        let (dir, storage) = setup();
        let envelope = json!({ "version": 99, "type": "marinara_character", "data": {} });
        let err = import_call(&storage, dir.path(), &["marinara"], envelope)
    ...
    #[test]
    fn import_call_rejects_unknown_route() {
        let (dir, storage) = setup();
