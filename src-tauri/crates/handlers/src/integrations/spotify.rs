...
// `src-tauri/src/commands/storage/integrations/spotify.rs` during Phase 4e
// so the Axum server target can drive the Spotify Web API (auth, playback,
// search, playlists, the "DJ Mari" generative playlist flow). The TCP
// loopback OAuth callback listener stays on the Tauri side
// (`commands/storage/integrations/spotify_callback.rs`) — it depends on
// localhost:8754 being reachable by the browser the user just opened.
// Phase 4f will add a new server-side HTTPS callback route so web users can
// complete the OAuth handshake without the desktop binary.
//
// The `ParsedPath` shape that the Tauri integration_call shim hands in
// lives here as a public type so both transports can construct one.
// `authorize` reports `callbackListenerStarted: false` from the handlers
// crate; the Tauri wrapper kicks off the loopback listener separately and
// patches the response.

use crate::shared::{
    find_by_field, get_required, percent_encode_component, required_string, string_array_from_value,
};
...
/// Path + query container the Spotify dispatcher uses to read fields like
/// `agentId` and `limit` out of either the URL or the request body. Same
/// shape the Tauri integration shim already constructed; lifted here so
/// the server-side dispatcher can build one inline.
...
pub async fn spotify_call(
    ...
    match (method, rest) {
        ("GET", ["authorize"]) | ("POST", ["authorize"]) => authorize(storage, route, &body),
        ("POST", ["exchange"]) => exchange(storage, body).await,
        ("POST", ["refresh"]) => {
            let agent_id = string_param(route, &body, "agentId")
                .ok_or_else(|| AppError::invalid_input("agentId is required"))?;
            refresh_agent_token(storage, &agent_id)
                .await
                .map(|_| json!({ "success": true }))
        }
        ("GET", ["status"]) | ("POST", ["status"]) => status(storage, route, &body),
        ("GET", ["access-token"]) => access_token(storage, route, &body).await,
        ("GET", ["player"]) => player(storage, route, &body).await,
        ("GET", ["devices"]) => devices(storage, route, &body).await,
        ("GET", ["playlists"]) => playlists(storage, route, &body).await,
        ("POST", ["playlist-tracks"]) => playlist_tracks(storage, body).await,
        ("POST", ["search-tracks"]) => search_tracks(storage, body).await,
        ("POST", ["play-track"]) => play_track(storage, body).await,
        ("POST", ["dj-mari-playlist"]) => dj_mari_playlist(storage, body).await,
        ("PUT", ["player", "play"]) => {
            player_control(storage, route, body, "/me/player/play", "PUT").await
        }
        ("PUT", ["player", "pause"]) => {
            player_control(storage, route, body, "/me/player/pause", "PUT").await
        }
        ("POST", ["player", "next"]) => {
            player_control(storage, route, body, "/me/player/next", "POST").await
        }
        ("POST", ["player", "previous"]) => {
            player_control(storage, route, body, "/me/player/previous", "POST").await
        }
        ("PUT", ["player", "volume"]) => player_volume(storage, route, body).await,
        ("PUT", ["player", "shuffle"]) => player_shuffle(storage, route, body).await,
        ("PUT", ["player", "repeat"]) => player_repeat(storage, route, body).await,
        ("PUT", ["player", "transfer"]) => player_transfer(storage, route, body).await,
        ("POST", ["disconnect"]) => disconnect(storage, body),
        _ => Err(AppError::new(
            "route_not_found",
...
async fn search_tracks(storage: &FileStorage, body: Value) -> AppResult<Value> {
    let query = body
        .get("query")
    ...
    let credentials = resolve_credentials(storage, &route, &body).await?;
...
async fn play_track(storage: &FileStorage, body: Value) -> AppResult<Value> {
    let track = body
        .get("track")
        .ok_or_else(|| AppError::invalid_input("track is required"))?;
    let device_id = body.get("deviceId").and_then(Value::as_str);
    game_spotify_play(storage, track, device_id).await
}

async fn playlist_tracks(storage: &FileStorage, body: Value) -> AppResult<Value> {
...
// Used only by `play_track` inside this module; pre-lift it was `pub(crate)`
// to allow future cross-module use that never materialized.
async fn game_spotify_play(
    storage: &FileStorage,
    track: &Value,
...
fn authorize(storage: &FileStorage, route: &ParsedPath, body: &Value) -> AppResult<Value> {
    ...
    // patches `callbackListenerStarted` after this call returns. Server
...
pub async fn exchange(storage: &FileStorage, body: Value) -> AppResult<Value> {
    let (code, auth_state) = spotify_code_and_state(&body)?;
    let key = format!("spotify-pending-{auth_state}");
    let pending_record = storage.get("app-settings", &key)?.ok_or_else(|| {
    ...
    if created_at > 0 && now_millis().saturating_sub(created_at) > AUTH_TTL_MS {
        let _ = storage.delete("app-settings", &key);
    ...
    .await?;
    let _ = storage.delete("app-settings", &key);
    save_spotify_tokens(storage, &agent_id, &client_id, &token)?;
    Ok(json!({ "success": true }))
}

fn status(storage: &FileStorage, route: &ParsedPath, body: &Value) -> AppResult<Value> {
    let agent_id = string_param(route, body, "agentId")
        .ok_or_else(|| AppError::invalid_input("agentId is required"))?;
    let agent = get_required(storage, "agents", &agent_id)?;
...
async fn access_token(storage: &FileStorage, route: &ParsedPath, body: &Value) -> AppResult<Value> {
    let credentials = resolve_credentials(storage, route, body).await?;
    Ok(json!({
...
async fn player(storage: &FileStorage, route: &ParsedPath, body: &Value) -> AppResult<Value> {
    let credentials = resolve_credentials(storage, route, body).await?;
...
async fn devices(storage: &FileStorage, route: &ParsedPath, body: &Value) -> AppResult<Value> {
    let credentials = resolve_credentials(storage, route, body).await?;
...
async fn playlists(storage: &FileStorage, route: &ParsedPath, body: &Value) -> AppResult<Value> {
    let credentials = resolve_credentials(storage, route, body).await?;
    let limit = route
        .query
        .get("limit")
...
async fn dj_mari_playlist(storage: &FileStorage, body: Value) -> AppResult<Value> {
    ...
    let context = build_dj_mari_context(storage, &playlist_name, &liked_songs)?;
    let generated_tracks = generate_dj_mari_playlist_plan(storage, &playlist_name, context).await?;
    let matched_tracks =
...
fn build_recent_chat_context(
    ...
    for mode in ["conversation", "roleplay", "game"] {
        ...
        let mut messages = crate::chats::messages_for_chat(storage, chat_id)?;
...
fn build_dj_mari_context(
    ...
    let mut chats = storage.list("chats")?;
    chats.sort_by(|a, b| {
        b.get("updatedAt")
            .and_then(Value::as_str)
            .unwrap_or("")
    ...
    let characters = storage.list("characters")?;
    let personas = storage.list("personas")?;
    ...
    Ok(json!({
        ...
        "characterNames": character_profiles.iter()
            ...
            .collect::<Vec<_>>(),
        "recentChats": build_recent_chat_context(storage, &chats, &character_names, persona_name.as_deref())?,
...
fn resolve_dj_mari_llm_connection(storage: &FileStorage) -> AppResult<Value> {
    let spotify_agent = find_spotify_agent(storage, None).ok();
...
async fn generate_dj_mari_playlist_plan(
    ...
    let connection = resolve_dj_mari_llm_connection(storage)?;
...
async fn player_volume(storage: &FileStorage, route: &ParsedPath, body: Value) -> AppResult<Value> {
    let credentials = resolve_credentials(storage, route, &body).await?;
    let volume = body
        .get("volume")
        .and_then(Value::as_i64)
...
async fn player_repeat(storage: &FileStorage, route: &ParsedPath, body: Value) -> AppResult<Value> {
    let credentials = resolve_credentials(storage, route, &body).await?;
...
fn disconnect(storage: &FileStorage, body: Value) -> AppResult<Value> {
    let agent_id = body
        .get("agentId")
        .and_then(Value::as_str)
        .ok_or_else(|| AppError::invalid_input("agentId is required"))?;
...
async fn resolve_credentials(
    ...
    let agent = find_spotify_agent(storage, string_param(route, body, "agentId").as_deref())?;
    let agent_id = agent
        .get("id")
        .and_then(Value::as_str)
        .unwrap_or("")
    ...
    {
        let token = refresh_agent_token(storage, &agent_id).await?;
        access_token = token
            .get("access_token")
            .and_then(Value::as_str)
            .unwrap_or("")
...
fn find_spotify_agent(storage: &FileStorage, preferred_agent_id: Option<&str>) -> AppResult<Value> {
    if let Some(id) = preferred_agent_id.filter(|id| !id.is_empty()) {
        let agent = get_required(storage, "agents", id)?;
        if agent.get("type").and_then(Value::as_str) == Some("spotify") || id == "spotify" {
            return Ok(agent);
        }
    }
    find_by_field(storage, "agents", "type", "spotify")?
        .ok_or_else(|| AppError::not_found("Spotify DJ agent is not configured."))
}

async fn refresh_agent_token(storage: &FileStorage, agent_id: &str) -> AppResult<Value> {
    let agent = get_required(storage, "agents", agent_id)?;
    let settings = agent_settings(&agent);
    let refresh_token = settings
        .get("spotifyRefreshToken")
...
mod tests {
    ...
    #[test]
    fn parsed_path_splits_parts_and_query() {
        let route = ParsedPath::new("/spotify/playlists?limit=25&agentId=mari");
        assert_eq!(route.parts, vec!["spotify", "playlists"]);
        assert_eq!(route.query.get("limit"), Some(&"25".to_string()));
        assert_eq!(route.query.get("agentId"), Some(&"mari".to_string()));
    }

    #[test]
    fn parsed_path_handles_empty_input() {
        let route = ParsedPath::new("");
        assert!(route.parts.is_empty());
        assert!(route.query.is_empty());
    }

    #[tokio::test(flavor = "current_thread")]
    async fn authorize_requires_client_and_agent_ids() {
        let (_dir, storage) = fresh();
        let route = ParsedPath::new("");
        let result = spotify_call(
            &storage,
            "POST",
            &["authorize"],
            &route,
        ...
        .await;
        let error = result.expect_err("clientId required");
        assert_eq!(error.code, "invalid_input");
    }

    #[tokio::test(flavor = "current_thread")]
    async fn authorize_returns_auth_url_and_persists_pending_session() {
        let (_dir, storage) = fresh();
        let route = ParsedPath::new("");
        let response = spotify_call(
            ...
            &["authorize"],
            &route,
            json!({ "clientId": "cid-123", "agentId": "mari" }),
        )
        .await
        .expect("authorize");
        // callbackListenerStarted is always false from the handlers crate;
        // the Tauri wrapper patches it.
        assert_eq!(
            response
                .get("callbackListenerStarted")
                .and_then(Value::as_bool),
            Some(false)
        );
        let url = response
        ...
        // The pending session row should now exist under a spotify-pending-* key.
        ...
        let pending = rows
            ...
            .expect("pending row");
        let value = pending.get("value").expect("value");
        assert_eq!(
            value.get("clientId").and_then(Value::as_str),
            Some("cid-123")
        );
        assert_eq!(value.get("agentId").and_then(Value::as_str), Some("mari"));
        assert!(value
            .get("codeVerifier")
            .and_then(Value::as_str)
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn exchange_rejects_missing_or_expired_session() {
        let (_dir, storage) = fresh();
        let error = exchange(
            &storage,
            json!({ "code": "auth-code", "state": "never-registered" }),
        )
        .await
    ...
    #[test]
    fn dispatcher_rejects_unknown_route() {
        // sync test using block_on so we cover the route-not-found arm.
        let (_dir, storage) = fresh();
        let route = ParsedPath::new("");
        let runtime = tokio::runtime::Builder::new_current_thread()
            .build()
            .expect("runtime");
        let result = runtime.block_on(spotify_call(
            &storage,
            "DELETE",
            &["nonsense"],
            &route,
        ...
        let error = result.expect_err("route not found");
