...
pub fn list_collection(
    ...
    let mut rows = match filter {
        Some((key, value)) => {
            ...
            storage.list_where(collection, &filters)?
        }
        None => storage.list(collection)?,
    };
    rows.sort_by(|a, b| {
        let a_order = a
            .get("sortOrder")
...
pub fn get_required(storage: &FileStorage, collection: &str, id: &str) -> AppResult<Value> {
    storage
        .get(collection, id)?
...
pub fn normalize_character_data_for_storage(data: &Value) -> AppResult<Value> {
    match data {
...
pub fn normalize_update_patch(collection: &str, patch: Value) -> AppResult<Value> {
...
pub fn with_entity_defaults(collection: &str, body: Value) -> Value {
...
pub fn duplicate_record(storage: &FileStorage, collection: &str, id: &str) -> AppResult<Value> {
    let mut record = get_required(storage, collection, id)?;
    let object = record
        .as_object_mut()
    ...
    storage.create(collection, record)
}
...
pub fn string_array_from_value(value: Option<&Value>) -> Vec<String> {
    match value {
...
pub fn metadata_map(chat: &Value) -> Map<String, Value> {
...
pub fn decode_uploaded_file_value(file: &Value) -> AppResult<UploadedFile> {
    let name = file
        .get("name")
        .and_then(Value::as_str)
...
pub fn decode_uploaded_file(body: &Value) -> AppResult<(String, String, Vec<u8>)> {
    let file = body
        .get("file")
...
pub fn decode_uploaded_files(body: &Value, field: &str) -> AppResult<Vec<UploadedFile>> {
...
pub fn upload_gallery_image(
    ...
    storage.create(collection, Value::Object(record))
}
