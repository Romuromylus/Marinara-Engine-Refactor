...
fn path_from_id(data_dir: &Path, category: &str, id: &str) -> AppResult<PathBuf> {
    ...
    if relative.contains("..") {
        return Err(AppError::invalid_input("Import id must not contain parent path segments"));
    }
...
pub(super) fn scan_st_folder(body: Value) -> AppResult<Value> {
    ...
    let characters: Vec<Value> = list_files(&data_dir.join("characters"), &[".json", ".png", ".charx"], false)
        .into_iter()
        .map(|path| {
    ...
    let chats: Vec<Value> = list_files(&data_dir.join("chats"), &[".jsonl"], true)
        ...
        .collect();
    let group_chats: Vec<Value> = list_files(&data_dir.join("group chats"), &[".jsonl", ".json"], true)
        .into_iter()
        .map(|path| {
        ...
        .map(|path| {
            ...
            if let Some(object) = item.as_object_mut() {
                ...
                object.insert(
                    ...
                    Value::Bool(matches!(
                        name.as_str(),
                        "default" | "deterministic" | "neutral" | "universal-creative" | "universal-light" | "universal-super-creative"
                    )),
                );
            }
            item
    ...
    persona_files.extend(list_files(&data_dir.join("User Avatars"), &[".json", ".txt"], false));
    persona_files.sort();
    persona_files.dedup();
...
fn import_st_chat_text(state: &AppState, text: &str, chat_name: String, inherited: Option<Value>) -> AppResult<Value> {
    ...
    chat.entry("mode".to_string()).or_insert(Value::String("chat".to_string()));
    chat.entry("characterIds".to_string()).or_insert_with(|| json!([]));
    chat.entry("metadata".to_string()).or_insert_with(|| json!({}));
    if !character_name.is_empty() {
    ...
    let chat_id = chat_record.get("id").and_then(Value::as_str).unwrap_or_default().to_string();
    let mut imported = 0usize;
    for row in parsed_rows {
        if row.get("is_system").and_then(Value::as_bool).unwrap_or(false) {
            continue;
        }
        let content = row
            .get("mes")
    ...
    Ok(json!({ "success": true, "chatId": chat_id, "chat": chat_record, "messagesImported": imported }))
}
...
pub(super) fn import_st_chat_into_group(state: &AppState, body: Value) -> AppResult<Value> {
    ...
    if let Some(object) = inherited.as_object_mut() {
        ...
        state.storage.patch("chats", target_chat_id, json!({ "groupId": group_id }))?;
    }
...
fn import_persona_payload(state: &AppState, payload: Value, fallback_name: &str) -> AppResult<Value> {
    let mut object = ensure_object(payload).unwrap_or_default();
    object.entry("name".to_string()).or_insert(Value::String(fallback_name.to_string()));
    if !object.contains_key("description") {
        if let Some(persona) = object.get("persona").or_else(|| object.get("content")).and_then(Value::as_str) {
            object.insert("description".to_string(), Value::String(persona.to_string()));
        }
    }
    state
        .storage
...
fn import_persona_file(state: &AppState, path: &Path) -> AppResult<Value> {
    ...
    let payload = parse_json_text(&raw).unwrap_or_else(|_| json!({ "name": fallback_name, "description": raw }));
...
fn copy_background_file(state: &AppState, path: &Path) -> AppResult<Value> {
    ...
    if final_target.exists() {
        ...
        for index in 1..10_000 {
            let candidate = state.backgrounds.root().join(format!("{stem}-{index}{ext}"));
            if !candidate.exists() {
...
pub(super) fn run_st_bulk_import(state: &AppState, body: Value) -> AppResult<Value> {
    ...
    let tag_mode = options.get("characterTagImportMode").and_then(Value::as_str).unwrap_or("all");
    ...
    for id in selected_ids(&options, "characters") {
        ...
        match result {
            ...
            Err(error) => errors.push(Value::String(format!("{}: {}", path.to_string_lossy(), error.message))),
        }
    }
    ...
    for id in selected_ids(&options, "lorebooks") {
        ...
        let result = fs::read(&path)
            ...
            .and_then(|payload| create_lorebook_from_payload(state, &payload, &file_stem(&path), None));
        match result {
    ...
    for id in selected_ids(&options, "presets") {
        ...
        let result = fs::read(&path)
            .map_err(AppError::from)
            .and_then(|bytes| parse_object(&bytes))
            .and_then(|payload| state.storage.create("prompts", with_entity_defaults("prompts", payload)));
        match result {
    ...
    for id in selected_ids(&options, "chats") {
        ...
        let result = fs::read_to_string(&path).map_err(AppError::from).and_then(|text| {
