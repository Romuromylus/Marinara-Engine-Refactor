...
pub(crate) fn collection_root(state: &AppState, method: &str, collection: &str, body: Value) -> AppResult<Value> {
    match method {
        "GET" => list_collection(state, collection, None),
        "POST" => state.storage.create(collection, with_entity_defaults(collection, body)),
        _ => Err(AppError::new("method_not_allowed", format!("{method} is not allowed for /{collection}"))),
    }
}
...
pub(crate) fn collection_item_or_action(
    ...
    match method {
        ...
        _ => Err(AppError::new("method_not_allowed", format!("{method} is not allowed for /{collection}/{id}"))),
    }
}

pub(crate) fn list_collection(state: &AppState, collection: &str, filter: Option<(&str, &str)>) -> AppResult<Value> {
...
pub(crate) fn handle_singleton(state: &AppState, method: &str, collection: &str, key: &str, body: Value) -> AppResult<Value> {
    match method {
        "GET" => Ok(state.storage.get(collection, key)?.unwrap_or_else(|| json!({ "id": key, "value": null }))),
        "PUT" | "PATCH" | "POST" => state.storage.upsert_with_id(collection, key, body),
        "DELETE" => {
            let deleted = state.storage.delete(collection, key)?;
            Ok(json!({ "deleted": deleted }))
        }
        _ => Err(AppError::new("method_not_allowed", "Unsupported singleton method")),
    }
}
...
pub(crate) fn with_entity_defaults(collection: &str, body: Value) -> Value {
    ...
    match collection {
        "chats" => {
            object.entry("metadata".to_string()).or_insert_with(|| json!({}));
            object.entry("gameState".to_string()).or_insert_with(|| json!({}));
            object.entry("characterIds".to_string()).or_insert_with(|| json!([]));
        }
        "connections" => {
        ...
        "characters" => {
            if let Some(data) = object.get_mut("data") {
                if data.is_object() {
                    *data = Value::String(serde_json::to_string(data).unwrap_or_else(|_| "{}".to_string()));
                }
            } else {
                ...
                data.insert("post_history_instructions".to_string(), Value::String(String::new()));
                data.insert("tags".to_string(), json!([]));
                data.insert("creator".to_string(), Value::String(String::new()));
                data.insert("character_version".to_string(), Value::String("1.0".to_string()));
                ...
                object.insert(
                    "data".to_string(),
                    Value::String(serde_json::to_string(&Value::Object(data)).unwrap_or_else(|_| "{}".to_string())),
                );
            }
            object.entry("comment".to_string()).or_insert(Value::String(String::new()));
            object.entry("avatarPath".to_string()).or_insert(Value::Null);
        }
        "lorebooks" => {
            object.entry("description".to_string()).or_insert(Value::String(String::new()));
            object.entry("category".to_string()).or_insert(Value::String("uncategorized".to_string()));
            object.entry("imagePath".to_string()).or_insert(Value::Null);
            object.entry("scanDepth".to_string()).or_insert(json!(2));
            object.entry("tokenBudget".to_string()).or_insert(json!(2048));
            object.entry("recursiveScanning".to_string()).or_insert(Value::Bool(false));
            object.entry("maxRecursionDepth".to_string()).or_insert(json!(3));
            object.entry("characterId".to_string()).or_insert(Value::Null);
            object.entry("characterIds".to_string()).or_insert(json!([]));
            ...
            object.entry("isGlobal".to_string()).or_insert(Value::Bool(false));
            object.entry("enabled".to_string()).or_insert(Value::Bool(true));
            object.entry("tags".to_string()).or_insert(json!([]));
            object.entry("generatedBy".to_string()).or_insert(Value::Null);
            object.entry("sourceAgentId".to_string()).or_insert(Value::Null);
        }
        "personas" => {
            object.entry("description".to_string()).or_insert(Value::String(String::new()));
            object.entry("comment".to_string()).or_insert(Value::String(String::new()));
            object.entry("personality".to_string()).or_insert(Value::String(String::new()));
            object.entry("scenario".to_string()).or_insert(Value::String(String::new()));
            object.entry("backstory".to_string()).or_insert(Value::String(String::new()));
            object.entry("appearance".to_string()).or_insert(Value::String(String::new()));
            object.entry("avatarPath".to_string()).or_insert(Value::Null);
            object.entry("isActive".to_string()).or_insert(Value::Bool(false));
            object.entry("tags".to_string()).or_insert(Value::String("[]".to_string()));
        }
        "prompts" => {
            object.entry("description".to_string()).or_insert(Value::String(String::new()));
            object.entry("parameters".to_string()).or_insert_with(|| json!({}));
            object.entry("isDefault".to_string()).or_insert(Value::Bool(false));
        }
        "agents" => {
            object.entry("enabled".to_string()).or_insert(Value::Bool(true));
        }
...
pub(crate) fn find_by_field(state: &AppState, collection: &str, field: &str, value: &str) -> AppResult<Option<Value>> {
    let mut filters = Map::new();
    filters.insert(field.to_string(), Value::String(value.to_string()));
    Ok(state.storage.list_where(collection, &filters)?.into_iter().next())
}

pub(crate) fn decode_path(value: &str) -> String {
    value.replace("%2F", "/").replace("%5C", "\\").replace("%20", " ")
}
...
pub(crate) fn decode_uploaded_files(body: &Value, field: &str) -> AppResult<Vec<UploadedFile>> {
    ...
    match value {
        ...
        _ => Err(AppError::invalid_input(format!("{field} must contain uploaded file objects"))),
    }
}
...
pub(crate) fn upload_gallery_image(
    ...
    record.insert(parent_field.to_string(), Value::String(parent_id.to_string()));
